...
action_planning_agent = ActionPlanningAgent(
    openai_api_key=openai_api_key, knowledge=knowledge
)
...
response = action_planning_agent.extract_steps_from_prompt(prompt=prompt)

# Print the response
print(response)

# Save to the same folder as this script
with open(os.path.join(os.path.dirname(__file__), "tests", "action_planning_agent.txt"), "w") as f:
    f.write(f"Prompt: \n{prompt}\n\n")
    f.write(f"Response: \n")
    f.write("\n".join(response))